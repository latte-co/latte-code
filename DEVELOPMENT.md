...
运行状态默认存放在 `$HOME/.latte/latte-code/state.db`，每个 Session 的追加式 JSONL 位于同一 Home 的 `sessions/`；可通过绝对路径环境变量 `LATTE_CODE_HOME` 覆盖 Product Home。构建输出位于 `target/`。
