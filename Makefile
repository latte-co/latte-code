...
coverage-unit: ## Run UT-only coverage with the 93% line gate
	cargo llvm-cov --workspace --all-features --lib --bins --locked --fail-under-lines 93

coverage-e2e: ## Run final-binary E2E coverage with the 88% line gate
	cargo llvm-cov --workspace --all-features --test e2e_portable --test e2e_unix --locked --fail-under-lines 88 -- --test-threads=1

coverage-total: ## Run all-target coverage with the 88% line gate
	cargo llvm-cov --workspace --all-features --all-targets --locked --fail-under-lines 88 -- --test-threads=1
