...
durable event 至少含 `event_id`、`session_id`、sequence、关联 Run、时间、typed redacted payload 与 source key。Run/Effect/Permission/Transcript 更新、revision 递增和 event append 必须原子提交；重复 source key 只产生一次状态变化和一次可见 event。
