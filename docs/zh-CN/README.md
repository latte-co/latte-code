...
- [全局 Session 与数据存储](design/data-storage.md)：已实现的全局状态路径、按 Workspace 隔离的 Session Catalog 与瞬态 Provider 配置失败，以及设计中的 JSONL、恢复和迁移契约。
- [斜杠命令](design/slash-commands.md)：已实现的 Built-in Catalog 与 Session 命令，以及设计中的 Composer Popup、Prompt Command 扩展和其余 Typed Action。
