...
Product state lives in `$LATTE_CODE_HOME`, defaulting to `$HOME/.latte/latte-code`. Global SQLite control state is `state.db`; each Session has an append-only JSONL transcript below `sessions/<workspace-storage-key>/`. `database.path` remains parseable only to locate an older workspace database for idempotent import. SQLite runs with WAL, foreign keys, a busy timeout, and full synchronous durability. The single writer commits an event, revision, and projection atomically. Command IDs are deduplicated across restart.

Thread v2 is additive: v1 `RunState`, commands, events, and protocol version remain byte compatible. Migration 7 adds separate `threads_v2`, linked child runs, the sole `thread_active_runs_v2` authority, paged typed transcript cards, a distinct thread event stream, and redacted command/source deduplication; migration 8 adds the engine-private canonical effect-descriptor table; migration 9 adds bounded Session catalog metadata (`title` and canonical `workspace_root`); migration 10 adds Session-scoped runtime leases while retaining globally monotonic fencing tokens; migration 11 adds global Project/Workspace registration, Session lineage metadata, and legacy-import fingerprints. JSONL is the only durable transcript read authority. SQLite uses a transactional `conversation_outbox` only until accepted entries have been synced to JSONL, then deletes those rows. A linked child cannot use a legacy transition, checkpoint, process, or tool mutation API. Its state/event/transcript updates are fenced through `CommitThreadRunUpdate`; old runs stay CLI-readable and are never backfilled.
...
Composer mode owns every printable character, including `q`, `s`, `j`, `k`, and `?`. Enter sends a nonblank composer or pending-input value, while Shift+Enter inserts a newline; the idle composer also advertises Ctrl+Enter as a compatibility submit chord, while F5 remains unadvertised. Unicode grapheme clusters are edited atomically, while wrapping, alignment, and cursor placement use terminal display width for CJK and emoji. Ctrl+P opens a local command palette whose help, navigation, refresh, and quit entries reduce to the same safe UI state or typed actions as their keyboard equivalents. The terminal session enables progressive keyboard disambiguation, bracketed paste, and mouse capture where supported, and restores the prior mode on every exit path. The mouse wheel scrolls transcript history. Esc explicitly enters transcript Navigation mode, where `j`/`k` select actions, PageUp/PageDown scroll, Enter/Space expands or collapses the selected action, and printable `q` quits; F10 is the explicit quit key in either mode. The first Ctrl+C interrupts active work and arms an exit confirmation; a second Ctrl+C within two seconds exits, while timeout or another key disarms it. A pending permission is an inline card pinned at the active waterfall tail immediately above the composer, with a bounded, control-safe redacted operation summary (write target/content intent, process argv/cwd, or a read/invocation target). Permission and reconciliation branches consume the complete key event: decisions require the exact focused `d` or Ctrl+A action, and Enter and Shift+Enter cannot approve, acknowledge, or mutate a text buffer. Event gaps and reconnects clear transient progress then reload an authoritative snapshot inside the projection adapter. The event loop redraws only after projected or local state changes. While a child is running, Enter can enqueue a bounded process-local follow-up; the Session's single runner consumes user inputs in FIFO order only after the current child reaches a runnable boundary. Independent Sessions use independent runners.

After submission, the composer immediately accepts the next draft, including Shift+Enter and paste, while another dispatch remains blocked until the submitted user card is durably reconciled.

A permission decision is armed only after the exact request identifier has reached a rendered frame. Input buffered before that frame cannot deny or approve an unseen operation.

The current thread coordinator supports provider conversation, typed user/assistant/input/failure/completion cards, immutable follow-up children, and exact bounded history. Provider-issued tool-call and input-request IDs must use the small opaque grammar `[A-Za-z0-9_-]{1,256}` before they can become durable source, request, or deduplication keys. It does not grant direct effect authority to the provider or TUI. Provider-requested v2 tools are executed only by the engine as fenced durable effects: `Prepare -> Started -> Observe`. The engine persists the descriptor before execution, renews the lease while a provider call or started effect is active, and records `Unknown` on restart or lease loss after `Started`; only an explicit reconciliation can resolve that state. Denying a prepared permission terminalizes that immutable child without executing the effect, returns the Session to `Ready`, and records bounded non-execution tool results when a later child reconstructs Provider history. The composer can therefore accept Shift+Enter and submit a new follow-up in the same Session. The TUI exposes unknown-effect acknowledgement as a separate confirmation: Ctrl+R opens the redacted card and Ctrl+A confirms; Enter never acknowledges it. Confirmation records the effect as failed and terminalizes the exact linked child.
...
- `latte-code` recursively merges built-in application defaults, `$HOME/.latte/latte-code.jsonc`, and workspace `.latte/latte-code.jsonc` in that order. Individual files may be absent and workspace keys win. Read-only state commands accept an empty Provider catalog, while TUI and Provider-backed operations require the merged configuration to explicitly provide the global `default_model` and a matching Provider model; Latte Code ships no built-in Provider catalog. A Provider's `models` value replaces the earlier layer as one complete catalog. CLI and TUI share one headless registry. Thread bindings structurally pin every v1 semantic binding field (including aliases), plus model options and internally derived non-secret credential/data-scope metadata before any secret resolution or history egress. These binding fields are not public Provider configuration. `context_window` remains pinned for subsequent context management and compaction. Resume fails closed when a pinned semantic binding no longer matches.
